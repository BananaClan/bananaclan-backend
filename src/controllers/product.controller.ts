...
export class ProductController {
  ...
  /**
 * Get top selling products, optionally filtered by brand
 */
static async getTopSellingProducts(req: Request, res: Response) {
  try {
    ...
    // Parse brand parameter (optional)
    let brands: string[] | undefined;
    if (req.query.brand) {
      // Handle single or multiple comma-separated brands
      const brandParam = req.query.brand as string;
      brands = brandParam.split(',').map(brand => brand.trim());
    }
    
    const productService = new ProductService();
    const result = await productService.getTopSellingProducts(page, limit, brands);
